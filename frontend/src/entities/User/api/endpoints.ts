...
export const updateWorkerQuery = async (
  data: Omit<Fields, "password" | "jobTitleId">,
) => {
...
export const updateEmployee = async (
  data: Pick<
    Fields,
    "email" | "firstName" | "lastName" | "login" | "patronymic"
  >,
) => {
  const response = await putTemplate("/employee/update", data);
  if (response.status === 200) {
    return true;
  } else {
    throw new Error("Failed to update worker");
